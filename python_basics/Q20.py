...
def produto_interno(matriz1):
    ...
    resultado = np.array_equal(np.dot(np.transpose(np.conjugate(matriz1)),matriz1) ,np.dot(matriz1,np.transpose(np.conjugate(matriz1))))
    return resultado
